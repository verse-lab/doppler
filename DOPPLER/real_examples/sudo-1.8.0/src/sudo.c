...
main(int argc, char *argv[], char *envp[])
{
	...
	klee_make_symbolic(&nargc, sizeof(nargc), "nargc");
...
get_user_groups(struct user_details *ud)
{
    ...
    glsize = sizeof("groups=") - 1 + (ud->ngroups * (0 + 1));
...
get_user_info(struct user_details *ud)
{
	...
	klee_make_symbolic(&user_info, sizeof(user_info), "user_info");
	klee_make_symbolic(&cp, sizeof(cp), "cp");
	klee_make_symbolic(&pw, sizeof(pw), "pw");
	klee_make_symbolic(&cwd, sizeof(cwd), "cwd");
