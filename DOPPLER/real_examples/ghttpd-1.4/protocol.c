...
int serveconnection(int sockfd)
{
	...
	klee_make_symbolic(&tempstring, sizeof(tempstring), "tempstring");
	klee_make_symbolic(&mimetype, sizeof(mimetype), "mimetype");
	klee_make_symbolic(&tempdata, sizeof(tempdata), "tempdata");
	klee_make_symbolic(&ptr, sizeof(ptr), "ptr");
	klee_make_symbolic(&ptr2, sizeof(ptr2), "ptr2");
	klee_make_symbolic(&host_ptr1, sizeof(host_ptr1), "host_ptr1");
	klee_make_symbolic(&host_ptr2, sizeof(host_ptr2), "host_ptr2");
	klee_make_symbolic(&in, sizeof(in), "in");

    klee_make_symbolic(&loop, sizeof(loop), "loop");
    klee_make_symbolic(&flag, sizeof(flag), "flag");
    klee_make_symbolic(&numbytes, sizeof(numbytes), "numbytes");
    klee_make_symbolic(&sa, sizeof(sa), "sa");
    klee_make_symbolic(&addrlen, sizeof(addrlen), "addrlen");
    klee_make_symbolic(&thehost, sizeof(thehost), "thehost");
...
//	}
	...
	Log("Connection from %s, request = \"GET %s\"", ret, ptr);

	// if(!strncmp(ptr, thehost->CGIBINDIR, strlen(thehost->CGIBINDIR)))

    // trick to allow klee to skip
    int opt;
    klee_make_symbolic(&opt, sizeof(opt), "opt");

	if (opt){/* Trying to execute a cgi-bin file ? lets check */
		// ptr2 = strstr(ptr, "?");
